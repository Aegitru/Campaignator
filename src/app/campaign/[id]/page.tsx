  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
      <div className="max-w-3xl w-full text-center">
        ...
        <div className="hud-label mb-3">SYSTEMS // {bundle.systems.length}</div>

        {bundle.systems.length === 0 ? (
          <div className="hud-panel--inset px-6 py-8 font-mono text-sm" style={{ color: "var(--text-faded)" }}>
            Aucun systeme cartographie. La Vue Galaxie multi-systemes arrive en V4.
            <br />Pour V3, les systemes seront crees via l'edition (a venir).
          </div>
        ) : (
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {bundle.systems.map((s) => (
              <li key={s.id}>
                ...
                >
                  <div className="font-display text-sm tracking-wide" style={{ color: "var(--accent-cyan)" }}>
                    {s.name}
                  </div>
