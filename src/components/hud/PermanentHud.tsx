...
/**
 * HUD permanent (cdc §8) — affiché sur toutes les vues sauf l'accueil ('/').
 * Contient : nom campagne, bouton CHRONO, barre de force, status_text, last_battle_summary.
 */
  ...
  return (
    ...
    >
      ...
      <div
        ...
        title="Cliquer pour détails par faction"
      >
      ...
      {campaign.last_battle_summary && (
        <div
          className="font-mono text-[11px] italic mb-1"
          style={{ color: "var(--accent-cyan)" }}
        >
          “{campaign.last_battle_summary}”
        </div>
...
function ForceBar({
  ...
  segments: { id: string; color: string; victories: number; label: string }[];
}) {
...
function AllianceRow({
  ...
  stats: {
    alliance: { id: string; name: string; color_hex: string };
    victories: number;
    factions: {
      faction: { id: string; name: string; color_hex: string };
      victories: number;
      battles: number;
    }[];
  };
      ...
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          <span
            ...
            style={{
              background: stats.alliance.color_hex,
              boxShadow: `0 0 6px ${stats.alliance.color_hex}`,
            }}
          />
          <span
          ...
          >
            <span className="flex items-center gap-1.5">
              <span
                ...
                style={{ background: f.faction.color_hex }}
              />
              {f.faction.name}
            </span>
            <span>
              {f.victories}/{f.battles}
            </span>
          </div>
        ))}
      </div>
    ...
    <div className="hud-panel--inset px-2.5 py-1.5 flex items-center justify-between">
      <div className="flex items-center gap-2">
        <span
          ...
          style={{
            background: stats.faction.color_hex,
            boxShadow: `0 0 6px ${stats.faction.color_hex}`,
          }}
        />
        <span
