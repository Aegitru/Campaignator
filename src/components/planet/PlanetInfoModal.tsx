      ...
      <div className="hud-panel hud-panel--strong max-w-xl w-full max-h-[88vh] overflow-hidden flex flex-col"
        ...
        <div className="px-6 pt-5 pb-3 flex items-start justify-between gap-3" style={{ borderBottom: "1px solid var(--border-faded)" }}>
          ...
          <div className="flex gap-2">
            {canEdit && !editing && (
              <button onClick={() => setEditing(true)} className="hud-button" style={{ padding: "0.4rem 0.6rem", fontSize: "1rem", lineHeight: 1 }}>✎</button>
            )}
        ...
        <div className="px-6 py-4 overflow-y-auto flex-1 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {editing ? (
                <select value={planetType} onChange={(e) => setPlanetType(e.target.value as any)}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              {editing ? (
                <label className="flex items-center gap-2 mt-2 font-mono text-xs cursor-crosshair" style={{ color: "var(--text-primary)" }}>
          ...
          <div>
            ...
            <ul className="space-y-1">
              {zones.map((z) => (
                <li key={z.id} className="hud-panel--inset px-3 py-1.5 font-mono text-xs flex justify-between items-center">
